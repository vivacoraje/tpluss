...
pub type SqlitePoolConnectionManager = Pool<RusqliteConnectionManager>;

#[derive(Clone)]
pub struct AppState {
    pub mssql_pool: PoolConnectionManager,
    pub sqlite_pool: SqlitePoolConnectionManager,
...
impl AppState {
    pub async fn new() -> anyhow::Result<Self> {
        ...
        let sqlite_mgr = RusqliteConnectionManager::new("delivery.db");
        let sqlite_pool = Pool::builder().max_size(3).build(sqlite_mgr).await?;
        ...
        Ok(Self { mssql_pool, sqlite_pool, group })
    }
}
